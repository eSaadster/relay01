...
export interface Attachment {
	...
	local: string; // absolute path to downloaded file
}
...
interface PendingDownload {
	absolutePath: string; // absolute path to download to
	url: string;
}
...
export class ChannelStore {
	 ...
	 * @param sessionName - Session name (e.g., "@username" or "#channelname") for scratchpad path
	 */
	processAttachments(
		...
		for (const file of files) {
			...
			// Download to session scratchpad if session name provided, otherwise legacy path
			let absolutePath: string;
			if (sessionName) {
				// Session scratchpad: ~/relay01/slack/{sessionName}/scratchpad/attachments/
				const scratchpadDir = join(process.env.HOME || "", "relay01", "slack", sessionName, "scratchpad", "attachments");
				absolutePath = join(scratchpadDir, filename);
			} else {
				// Fallback to legacy path
				absolutePath = join(this.workingDir, "attachments", filename);
			}
			...
			this.pendingDownloads.push({ absolutePath, url });
		}
	...
	private async processDownloadQueue(): Promise<void> {
		...
		while (this.pendingDownloads.length > 0) {
			...
			try {
				await this.downloadAttachment(item.absolutePath, item.url);
			} catch (error) {
				const errorMsg = error instanceof Error ? error.message : String(error);
				log.logWarning(`Failed to download attachment`, `${item.absolutePath}: ${errorMsg}`);
			}
		}
	...
	/**
	 * Download a single attachment to an absolute path
	 */
	private async downloadAttachment(absolutePath: string, url: string): Promise<void> {
